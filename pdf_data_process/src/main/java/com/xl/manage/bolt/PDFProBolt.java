...
import com.xl.manage.service.PDFProService;
 ...
 * @Date:15:29 2018/3/7
 */
public class PDFProBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		PDFProService service = new PDFProService();

		try{
			webBean = service.changeImage(bean);
			if(webBean!=null){
				...
				LogHelper.info("-------------------------------"+ CommonData.LOG_OUT_INFO+"PDF图片处理bolt失败！WebOldBean["+ JSONObject.toJSONString(bean)+"]");
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.info("----------------------"+ CommonData.LOG_OUT_INFO+"PDF图片处理bolt出现异常！DataProBolt.execute; WebOldBean["+ JSONObject.toJSONString(bean)+"]"+e.getMessage());
		}
	}
