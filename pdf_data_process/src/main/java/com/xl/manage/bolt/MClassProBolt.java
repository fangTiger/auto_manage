 ...
 */
public class MClassProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(MClassProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------"+ CommonData.LOG_OUT_INFO+"获取监测项bolt失败！ArticleBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.info("----------------------"+ CommonData.LOG_OUT_INFO+"获取监测项bolt出现异常！MClassProBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------"+e.getMessage());
		}
	}
