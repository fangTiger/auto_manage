...
import com.xl.manage.service.FingerprintService;
 ...
 */
public class FingerprintBolt extends BaseRichBolt {
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(FingerprintBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------"+ CommonData.LOG_OUT_INFO+"语义指纹处理失败！ArticleBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.info("----------------------"+ CommonData.LOG_OUT_INFO+"语义指纹处理出现异常！FingerprintBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------"+e.getMessage());
		}
	}
