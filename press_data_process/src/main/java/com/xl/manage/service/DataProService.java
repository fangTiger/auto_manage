...
public class DataProService {
	...
	public ArticleBean dealData(WebOldBean oldBean){
		...
		try{
			...
			if(oldBean.getK_article().getAid()==0||oldBean.getK_article().getAid()==10101010){
				aid = HeritrixHelper.getAidByUrl(StringUtil.toTrim(oldBean.getK_article().getUrl()).toLowerCase())+"";
			}else{
				aid = oldBean.getK_article().getAid()+"";
			}


			if(isRepeat(aid,"","press")){//重复数据
	...
	private void initShieldDictionary(){
		...
		try{
			...
			if(shield_dictionary_stemp!=new_stemp){
				//当时间戳变更，重新获取替换词
				result = HttpClientPoolUtil.execute(CommonData.SHIELD_DICTIONARY_URL.replace("MEDIA_VALUE","web"));
				//解析结果并放入PublicClass类的公共变量中
				jsonObject = JSON.parseObject(result);
				if("success".equals(StringUtil.toTrim(jsonObject.getString("msg")))){
					jsonArray = jsonObject.getJSONArray("datas");
	...
	public ArticleBean changeBean(WebOldBean oldBean)throws Exception{
		...
		page = PublicClass.replaceFirst(StringUtil.toTrim(oldBean.getK_article().getPage()),"0");//OCR数据page字段要去掉前面0 例如:0017 变成17
		bean.setProportion(Double.parseDouble(StringUtil.toTrim(oldBean.getK_article().getM1()+"")));//OCR将版面占比存到M1字段
		bean.setAuthor(oldBean.getK_article().getAuthor().split(";")[0].replace("/",""));//OCR记着多值用,分割 原行【中文/中文;英文/英文】

		bean.set_size(oldBean.getK_article().getM2());

		if("2".equals(StringUtil.toTrim(bean.getCrawlType()))){//ocr上传数据
			double size = 0;
			if(oldBean.getK_docpsimage()!=null&&oldBean.getK_docpsimage().size()>0){
				for(Docpsimagebean docpsimagebean:oldBean.getK_docpsimage()){
					if(docpsimagebean.getType()==3){ //只取类型为3的图片
						size += docpsimagebean.getHeight()*docpsimagebean.getWidth();
					}
				}
				if(size>0){
					bean.set_size(size);
		...
		JSONObject sitData;
		try{
			resultSite = HttpClientPoolUtil.execute(CommonData.GET_SITE_DOMAIN_API.replace("URL_VALUE",URLEncoder.encode(oldBean.getK_article().getUrl(),"utf-8")));
		...
		bean.setContentText(PublicClass.replaceContentText(PublicClass.replaceO(oldBean.getK_article().getHtmlPage(),"contenttext")));
		...
		bean.set_content(oldBean.getK_article().getHtmlPage());
	...
	public static void main(String[] args) {
		String htmlStr = "<p>跟维秘天使登顶帝国大厦，感受纽约时尚地标的魅力<img src=\"11111111\">safgsgdfhgdhdf</img></p>";
...
//
//		String result = HttpClientPoolUtil.execute(CommonData.GET_SIG_SIGALL_API,jsonObject.toJSONString());
