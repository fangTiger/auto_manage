...
import com.xl.manage.service.WebFingerprintService;
 ...
 */
public class WebFingerprintBolt extends BaseRichBolt {
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(WebFingerprintBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		WebFingerprintService service = new WebFingerprintService();
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------Web语义指纹处理失败！ArticleBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.info("----------------------Web语义指纹处理出现异常！WebFingerprintBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------"+e.getMessage());
		}
	}
