...
import com.xl.manage.service.WebInsertProService;
 ...
 */
public class WebInsertProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(WebInsertProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		WebInsertProService service = new WebInsertProService();
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------数据入库失败！ArticleBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.info("----------------------Web入库bolt出现异常！WebInsertProBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------"+e.getMessage());
		}
	}
