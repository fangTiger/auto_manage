 ...
 */
public class WebAffectionBolt extends BaseRichBolt {
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(WebAffectionBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------web情感值处理失败！ArticleBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.info("----------------------web情感值处理出现异常！WebAffectionBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------"+e.getMessage());
		}
	}
