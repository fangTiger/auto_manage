...
public class WebDataProService {
	...
	public ArticleBean dealData(WebOldBean oldBean){
		...
		try{
			...
			//TODO 测试库不再排重
			/*if(isRepeat("",oldBean.getK_article().getUrl(),"web")){//重复数据
	...
	public ArticleBean changeBean(WebOldBean oldBean)throws Exception{
		...
		try{
			...
			LogHelper.error(LogCommonData.LOG_CODE_WEB,"根据url获取媒体site和domain", oldBean.getK_article().getUrl(),"获取文章的域名和站点出现异常！",e);
			return null;
		}
