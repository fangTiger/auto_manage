...
/**
 * @Author:lww
 * @Date:17:07 2017/9/11
 */
public class AutoSubSourceService {

	//信息源规则时间戳缓存
	public static long RULE_SOURCE_STAMP = 0;

	//信息源规则缓存
	public static List<SubjectRuleBean> RULE_SOURCE_LIST = new ArrayList<>();
	...
	public ArticleBean getSourceRule(ArticleBean articleBean)throws Exception{
		...
		List<SubjectRuleBean> ruleList = new ArrayList<>();
		List<SubjectRuleBean> list = getRuleList();
		if(list!=null&&!list.isEmpty()){
			for(SubjectRuleBean ruleBean:list){
			...
			articleBean.set_subjectSourceList(ruleList);
		}
		return articleBean;
	}
	 ...
	 * @param
	 */
	private List<SubjectRuleBean> getRuleList()throws Exception{
		...
		try{
			...
			stamp = HttpClientPoolUtil.execute(AutoCommonData.getRuleStempUrl(0,1));//TODO 调用信息源规则时间戳API
			stampLong = StringUtil.stringToLong(stamp);
			if(stampLong>RULE_SOURCE_STAMP){//更新规则

				rule = HttpClientPoolUtil.execute(AutoCommonData.getRuleUrl(0,1));//TODO 调用信息源规则API
				if(!"".equals(rule)){
