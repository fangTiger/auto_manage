 ...
 */
public class AutoEveInsertService {
	...
	public boolean insertBean(ArticleBean bean)throws Exception{
		...
		String result = "";
		try{
			eventList = this.getEventData(bean);
			if(eventList==null){
				return false;//处理失败
			}else{
			...
			LogHelper.error("---------------活动数据入库数据出现异常！AutoEveInsertService.insertBean--------------------",e);
			return false;
		}
	}
	 ...
	 * @param bean
	 */
	private List<EventArticleBean> getEventData(ArticleBean bean)throws Exception{

		List<EventArticleBean> list = new ArrayList<>();

		EventRuleBean ruleBean;
		EventArticleBean articleBean;
		List<EventRuleBean> eventSourceList;
		List<EventRuleBean> eventContentList;

		List<Integer> reId = new ArrayList<>();//排重
		try{
			eventSourceList = bean.get_eventSourceList();
			eventContentList = bean.get_eventContentList();

			if(eventSourceList!=null&&!eventSourceList.isEmpty()){
				for(EventRuleBean sourceRuleBean:eventSourceList){
					ruleBean = null;
					if(eventContentList!=null&&!eventContentList.isEmpty()){
						for(EventRuleBean contentRuleBean:eventContentList){
					...
					//获取转换对象
					articleBean = getEventBean(bean,sourceRuleBean,ruleBean);
			...
			if(eventContentList!=null&&!eventContentList.isEmpty()){
				for(EventRuleBean contentRuleBean:eventContentList){
					if(!reId.contains(contentRuleBean.getId())){
						articleBean = getEventBean(bean,null,contentRuleBean);
						list.add(articleBean);
					}
				}
			}
			...
			list = null;
			LogHelper.error("---------------获取活动入库数据出现异常！AutoEveInsertService.getEventData--------------------",e);
		}
		return list;
	}
	 ...
	 */
	private EventArticleBean getEventBean(ArticleBean bean,EventRuleBean sourceRuleBean,EventRuleBean contentRuleBean)throws Exception{

		EventArticleBean articleBean = new EventArticleBean();
		...
		if(sourceRuleBean!=null){
			articleBean.setEventId(sourceRuleBean.getEventId());
			...
			articleBean.setEventId(contentRuleBean.getEventId());
