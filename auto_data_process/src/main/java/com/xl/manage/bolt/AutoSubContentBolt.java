...
import com.xl.manage.service.AutoSubContentService;
 ...
 * @Date:11:23 2017/9/8
 */
public class AutoSubContentBolt  extends BaseRichBolt {

	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(AutoSubContentBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		AutoSubContentService service = new AutoSubContentService();
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-----------------------主题获取内容规则失败！AutoSubContentBolt.execute ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------主题获取内容规则出现异常！AutoSubContentBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
