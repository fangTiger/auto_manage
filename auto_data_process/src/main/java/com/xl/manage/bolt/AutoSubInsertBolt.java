 ...
 * @Date:11:22 2017/9/8
 */
public class AutoSubInsertBolt extends BaseRichBolt {

	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(AutoSubInsertBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		AutoSubInsertService service = new AutoSubInsertService();
		...
		try{
			...
			if(result){
				...
				LogHelper.info("-----------------------主题数据入库失败！AutoSubInsertBolt.execute ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------主题数据入库出现异常！AutoSubInsertBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
