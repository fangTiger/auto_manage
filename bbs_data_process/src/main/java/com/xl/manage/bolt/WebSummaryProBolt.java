...
import com.xl.manage.service.WebSummaryService;
 ...
 * 关键词摘要处理
 * @Author:lww
 * @Date:11:22 2017/11/3
 */
public class WebSummaryProBolt  extends BaseRichBolt {

	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(WebSummaryProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("------------------------------获取关键词摘要失败！ArticleBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.info("----------------------web获取关键词摘要bolt出现异常！WebSummaryProBolt.execute; ArticleBean["+ JSONObject.toJSONString(bean)+"]--------------------------"+e.getMessage());
		}
	}
