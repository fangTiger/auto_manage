...
public class WebDataProService {
	...
	public ArticleBean dealData(WebOldBean oldBean){
		...
		try{
			...
			//TODO 测试库不再排重
			if(isRepeat("",oldBean.getK_article().getUrl(),"app")){//重复数据
	...
	private boolean isQualifiedArticle(WebOldBean oldBean)throws Exception{
		...
		if("".equals(StringUtil.toTrim(reason))&&!StringUtil.toTrim(oldBean.getK_article().getUrl()).toLowerCase().startsWith("http")){
			reason = "文章链接错误！";
		}
	...
	private void insertErrorDb(WebOldBean oldBean,String reason){
		...
		Long aid;
		try{
			aid = HeritrixHelper.getAidByUrl(oldBean.getK_article().getUrl().toLowerCase());
	...
	private void initShieldDictionary(){
		...
		try{
			...
			if(shield_dictionary_stemp!=new_stemp){
				//当时间戳变更，重新获取替换词
				result = HttpClientPoolUtil.execute(WebCommonData.SHIELD_DICTIONARY_URL.replace("MEDIA_VALUE","web"));
				//解析结果并放入PublicClass类的公共变量中
				jsonObject = JSON.parseObject(result);
				if("success".equals(StringUtil.toTrim(jsonObject.getString("msg")))){
					jsonArray = jsonObject.getJSONArray("datas");
	...
	public ArticleBean changeBean(WebOldBean oldBean)throws Exception{
		...
		if("".equals(StringUtil.toTrim(oldBean.getK_article().getDispTime()))||"null".equals(StringUtil.toTrim(oldBean.getK_article().getDispTime()))||"1900/01/01 00:00:00".equals(StringUtil.toTrim(oldBean.getK_article().getDispTime()))){
		...
		try{
			...
			String re = JSON.toJSONString(oldBean);
			LogHelper.error(LogCommonData.LOG_CODE_WEB,"获取媒体site和domain", oldBean.getK_article().getUrl(),"获取文章的域名和站点出现异常！",e);
			return null;
		}
		...
		if("".equals(StringUtil.toTrim(bean.getMedia().getMediaNameCn()))){
			bean.getMedia().setMediaNameCn(bean.getMedia().getDomain());
		}

		if("".equals(StringUtil.toTrim(bean.getMedia().getMediaNameEn()))){
