 ...
 */
public class TfTClassProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(TfTClassProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("------------------------------获取标签失败！TfBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------TF获取标签bolt出现异常！TfTClassProBolt.execute; TfBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
