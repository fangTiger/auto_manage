 ...
 */
public class TfAffectionBolt  extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		TfAffectionService service = new TfAffectionService();
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------Tf情感值处理失败！TfBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------Tf情感值处理出现异常！TfAffectionBolt.execute; TfBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
