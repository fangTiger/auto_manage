...
import com.xl.tf.service.TfClassifierService;
 ...
 */
public class TfMClassProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(TfMClassProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		TfClassifierService service = new TfClassifierService();
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------获取监测项失败！TfBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------TF获取监测项bolt出现异常！TfMClassProBolt.execute; TfBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
