 ...
 */
public class TfWClassProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(TfWClassProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------获取预警失败！TfBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------TF获取预警bolt出现异常！TfWClassProBolt.execute; TfBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
