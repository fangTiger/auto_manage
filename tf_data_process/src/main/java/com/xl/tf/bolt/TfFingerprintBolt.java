...
import com.xl.tf.service.TfFingerprintService;
 ...
 */
public class TfFingerprintBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		TfFingerprintService service = new TfFingerprintService();
		try{
			...
			if(bean!=null){
				...
				LogHelper.info("-------------------------------Tf语义指纹处理失败！TfBean["+ JSONObject.toJSONString(bean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			outputCollector.fail(tuple);
			LogHelper.error("----------------------Tf语义指纹处理出现异常！TfFingerprintBolt.execute; TfBean["+ JSONObject.toJSONString(bean)+"]--------------------------",e);
		}
	}
