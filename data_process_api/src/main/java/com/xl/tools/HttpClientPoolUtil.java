			...
			while (it.hasNext()) {
				...
				if (value != null && param.equalsIgnoreCase("timeout")) {
					try {
						...
						e.printStackTrace();
						LogHelper.info("format KeepAlive timeout exception, exception:" + e.toString());
					}
				}
			}
...
//				logger.info("请求URL: "+uri+"+  返回状态码："+httpResponse.getStatusLine().getStatusCode());
			...
			e.printStackTrace();
			LogHelper.info("execute get request exception, url:" + uri + ", exception:" + e.toString() + ",cost time(ms):"
					+ (System.currentTimeMillis() - startTime));
		} finally {
			if (httpEntity != null) {
				try {
					...
					e.printStackTrace();
					LogHelper.info("close response exception, url:" + uri + ", exception:" + e.toString() + ",cost time(ms):"
							+ (System.currentTimeMillis() - startTime));
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		obj.put("sinKafValue","{\"platformCode\":\"iSpider111\",\"moduleCode\":\"iSpiderCrawlTask\",\"operationCode\":\"wx_keyword_search11\",\"operationName\":\"iSpider采集任务11111\",\"describe\":\"\",\"state\":0,\"createTime\":\"2017/11/10 14:38:34\",\"dataContent\":\"{\\\"apiType\\\":\\\"weixin\\\",\\\"functionType\\\":\\\"wx_keyword_search\\\",\\\"taskId\\\":\\\"26993\\\",\\\"flag\\\":\\\"1\\\"}\",\"errorDescribe\":\"\"}");
		System.out.println(HttpClientPoolUtil.execute("http://192.168.10.72:8083/kafkaConsume/kafkaApi/putKafkaData",obj.toJSONString()));;
	}
}
