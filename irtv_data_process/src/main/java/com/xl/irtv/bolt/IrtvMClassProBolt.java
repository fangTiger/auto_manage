 ...
 */
public class IrtvMClassProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(IrtvMClassProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			broadcastBean = service.dealClass(broadcastBean,"1");
			if(broadcastBean!=null){
				...
				LogHelper.info("-------------------------------获取监测项失败！BroadcastBean["+ JSONObject.toJSONString(broadcastBean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.error("----------------------Irtv获取监测项bolt出现异常！IrtvMClassProBolt.execute; BroadcastBean["+ JSONObject.toJSONString(broadcastBean)+"]--------------------------",e);
		}
	}
