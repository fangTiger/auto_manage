...
import com.xl.irtv.service.IrtvExtractionProService;
 ...
 */
public class IrtvExtractionProBolt extends BaseRichBolt {
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(IrtvExtractionProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		IrtvExtractionProService service = new IrtvExtractionProService();

		BroadcastBean broadcastBean = (BroadcastBean)tuple.getValueByField(MessageScheme.STRING_SCHEME_KEY);
		try{
			broadcastBean = service.dealExtraction(broadcastBean);
			if(broadcastBean!=null){
				...
				LogHelper.info("-------------------------------实体抽取失败！BroadcastBean["+ JSONObject.toJSONString(broadcastBean)+"]---------------------------------------------");
				outputCollector.fail(tuple);
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.error("----------------------Irtv实体抽取bolt出现异常！IrtvExtractionProBolt.execute; BroadcastBean["+ JSONObject.toJSONString(broadcastBean)+"]--------------------------",e);
		}
	}
