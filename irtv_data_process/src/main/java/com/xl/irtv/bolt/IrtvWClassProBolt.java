 ...
 */
public class IrtvWClassProBolt extends BaseRichBolt{
	private OutputCollector outputCollector;
	private static final Logger log = LoggerFactory.getLogger(IrtvWClassProBolt.class);
	...
	public void execute(Tuple tuple) {
		...
		try{
			broadcastBean = service.dealClass(broadcastBean,"3");
			if(broadcastBean!=null){
				...
				LogHelper.info("-------------------------------获取预警失败！BroadcastBean["+ JSONObject.toJSONString(broadcastBean)+"]---------------------------------------------");
			}
		}catch (Exception e){
			AtomicIntegerBean.reduceAndGet();
			outputCollector.fail(tuple);
			LogHelper.error("----------------------Irtv获取预警bolt出现异常！IrtvWClassProBolt.execute; BroadcastBean["+ JSONObject.toJSONString(broadcastBean)+"]--------------------------",e);
		}
	}
